...
- <b>Should multiple JPEG images bearing the same root name be placed in the "OCR Raw Data" folder (for example: "scan.jpg", "scan0001.jpg", "scan0002.jpg"), the OCR prediction results for each page will be added in sequence to the correspondingly named "scan.txt" file, within the "scan" subfolder of the "OCR Predictions" folder. Should you ever want to repeat the OCR prediction for a set of JPEG images, it would then be important to remove the "scan" subfolder before running the "brl-slate-reader.py" code once more, in order to avoid appending more text to the existing "scan.txt" file.
...
- The first thing that the code will do is perform image segmentation (determine the x and y coordinates of every braille character). The segmentation results are visible as green rectangles overlaid onto the scanned image, in the "Page image files with rectangles" folder, which is created automatically by the code. Upon inspection of the green rectangles, you might wish to fine-tune the horizontal and vertical spacing in-between Braille cells, such that every green rectangle would frame their Braille cell nicely. Simply adjust the value of "horizontal_spacer_pixels:" and "vertical_spacer_pixels:" until you get satisfactory results, and the code will keep these optimal settings in memory in the text file found within the "Default_Parameters" folder. You should make a backup of these optimal parameters, and maybe add a label on your slate with these values, so that you don't need to repeat the calibration step in the future for this slate. When running the code with a different slate, you would either need to calibrate it, or replace the "Default_Parameters.txt" calibration file (if present) for the file you created previously for this slate.   
...
- The following RTF commands are automatically converted into BRF tags by the code and are transcribed from braille to English RTF commands in the RTF file: 
  ...
  - New section RTF commands "\sbkpage" ("⠸⠡⠎⠃⠅⠏⠁⠛⠑") will result in the creation of several BRF files (one for each section, which will be placed within a BRF folder within the "OCR Predictions" subfolder under your file.
